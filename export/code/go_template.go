...
var templateGoStruct = template.Must(template.New("go_struct").
	...
	{{range $index,$field := .Struct.Fields}}{{if $index}}{{"\n"}}{{end}}{{$.Exporter.GenStructField $field}}{{end}}
}
`))
...
func (e *goExporter) GenTableUniqueKeyMethod(td *parse.Table, field *parse.TableField) string {
	var sb strings.Builder
...
func (e *goExporter) GenTableCompositeKeyInitMethod(td *parse.Table, ck *parse.TableCompositeKey) string {
	...
	if err := templateGoTableCompositeKeyInitMethod.Execute(&sb, map[string]interface{}{
		...
		panic(pkg_errors.WithMessage(err, "export code: go: GenTableCompositeKeyInit"))
	}
	return sb.String()
}
