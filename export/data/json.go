...
func (e *jsonExporter) marshalJsonTableEntry(td *parse.Table, entry parse.TableEntry) ([]byte, error) {
	n := 0
	...
	for _, fd := range td.Fields {
		...
		if fd.Col == define.TableColFieldID {
			buf.WriteString(e.jsonFieldNamePrefix(define.TableFieldIDJsonTagName))
		} else {
...
func (e *jsonExporter) marshalJsonFieldValue(fd *parse.Field, val interface{}) ([]byte, error) {
	if fd.Type.Primitive() {
		...
		panic(fmt.Sprintf("marshalFieldValue: field type %d invalid"))
	}
}
...
func (e *jsonExporter) marshalJsonStructValue(sd *parse.Struct, val interface{}) ([]byte, error) {
	...
	for _, fd := range sd.Fields {
		refFieldVal := refVal.FieldByName(parse.GenStructFieldName(fd.Name))
...
func (e *jsonExporter) marshalJsonArrayValue(fd *parse.Field, val interface{}) ([]byte, error) {
	if fd.ElementType.Primitive() {
		...
		panic(fmt.Sprintf("export data: json: marshalArrayValue: elment type %d invalid"))
	}
}
